...
function ShowDownLoad() {
  console.log('如果超过5秒无响应，请点此<a target="_blank" href="' + downloadUrl + '" + date.getMinutes() + date.getSeconds()">安装</a>');
}
...
function StartCapture() {
  ...
  //从返回值来解析显示
  if (captureRet == niuniu.emCaptureFailed) {
    ShowDownLoad();
  } else if (captureRet == niuniu.emCaptureUnknown) {
    console.log('正在截图中, 如果超过5秒无响应，请点此<a target="_blank" href="' + downloadUrl + '" + date.getMinutes() + date.getSeconds()">安装</a>');
  }
}
...
function OnCaptureFinishedCallback(type, x, y, width, height, info, content, localpath) {
  ...
  console.log(info == '0' ? '从剪贴板获取图片失败。' : str[type])
}
